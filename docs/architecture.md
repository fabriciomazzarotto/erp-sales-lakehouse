...
**Local (implementado e validado — ver [`docs/automation.md`](automation.md)):** duas tarefas no Windows Task Scheduler, sem depender de AWS. Uma simula um dia de atividade no ERP de origem (novas notas/itens/updates, `sql/05_simulate_daily_activity.sql`); a outra roda o pipeline completo 15 minutos depois (`Bronze → Silver → Gold → Diamond → publicação em ERP_Sales_BI`, ver [`powerbi/README.md`](../powerbi/README.md)). O banco `ERP_Sales_BI` (SQL Server local, separado do banco de origem do ERP) é o que permite ao Power BI ler os dados via conector nativo — e agendar atualização real no Power BI Service através do Gateway de Dados Local, sem depender de AWS. Rodar isso de verdade (repetidamente, com dados "de agora") expôs e permitiu corrigir 3 bugs reais que o seed histórico original nunca tinha exposto — um bug de precisão no predicado incremental, um bug de timezone (Spark interpretando timestamps UTC do SQL Server como hora local), e falhas de orquestração no PowerShell — todos documentados em `docs/automation.md`.
